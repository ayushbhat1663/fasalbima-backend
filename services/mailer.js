...
async function sendInsuranceRequestEmail(data, retryCount = 0) {
    ...
    try {
        // PRE-FLIGHT DIAGNOSTICS WITH RAW CONNECTIVITY TESTING
        console.log('\n🔍 ═══════════════════════════════════════════════════════════');
        console.log('🔍 PRE-FLIGHT GMAIL TRANSPORTER DIAGNOSTICS');
        console.log('🔍 ═══════════════════════════════════════════════════════════');
        
        // LAYER 1: Show actual transporter configuration
        console.log('\n📋 LAYER 1: Transporter Configuration (ACTUAL VALUES):');
        console.log(`  • Host: smtp.gmail.com`);
        console.log(`  • Port: 465 (SMTPS with implicit TLS)`);
        console.log(`  • Secure: true (TLS from connection start)`);
        ...
        // LAYER 2: Test DNS resolution
        console.log('\n🌐 LAYER 2: DNS Resolution Test...');
        const dns = require('dns').promises;
        let dnsResult;
        try {
            dnsResult = await dns.resolve4('smtp.gmail.com');
            console.log(`✅ DNS Resolution SUCCESS: smtp.gmail.com → ${dnsResult.join(', ')}`);
        } catch (dnsError) {
            console.error(`❌ DNS Resolution FAILED: ${dnsError.message}`);
            throw new Error(`DNS resolution failed for smtp.gmail.com: ${dnsError.message}`);
        }
        
        // LAYER 3: Test raw TCP connection
        console.log('\n🔌 LAYER 3: Raw TCP Connection Test (smtp.gmail.com:465)...');
        const net = require('net');
        let tcpConnected = false;
        let tcpError = null;
        
        const tcpTest = new Promise((resolve, reject) => {
            const socket = net.createConnection({ host: 'smtp.gmail.com', port: 465, timeout: 15000 });
            
            socket.on('connect', () => {
                console.log(`✅ TCP Connection SUCCESS: Connected to smtp.gmail.com:465`);
                tcpConnected = true;
                socket.destroy();
                resolve();
            });
            ...
            socket.on('timeout', () => {
                console.error(`❌ TCP Connection TIMEOUT: No response after 15000ms`);
                tcpError = 'TCP connection timeout';
                socket.destroy();
        ...
        try {
            ...
            console.error(`⚠️ TCP test failed, but continuing to SMTP verification...`);
        }
        
        // LAYER 4: Test TLS handshake with explicit TLS socket
        console.log('\n🔐 LAYER 4: TLS Handshake Test (smtp.gmail.com:465 SMTPS)...');
        ...
        const tlsTest = new Promise((resolve, reject) => {
            ...
            const socket = tls.connect(options, () => {
                console.log(`✅ TLS Handshake SUCCESS: TLS connection established to smtp.gmail.com:465`);
                tlsConnected = true;
                socket.destroy();
                resolve();
            });
            ...
            socket.on('timeout', () => {
                console.error(`❌ TLS Handshake TIMEOUT: No response after 15000ms`);
                tlsError = 'TLS handshake timeout';
                socket.destroy();
                reject(new Error('TLS handshake timeout'));
            });
            
            socket.on('error', (err) => {
                console.error(`❌ TLS Handshake ERROR: ${err.code || err.name} - ${err.message}`);
                tlsError = `${err.code || err.name}: ${err.message}`;
                reject(err);
            });
        });
        ...
        try {
            ...
            console.error(`⚠️ TLS test failed, but continuing to SMTP verification...`);
        }
        
        // LAYER 5: Nodemailer transporter verification (SMTP protocol negotiation)
        console.log('\n📧 LAYER 5: Nodemailer SMTP Verification (Protocol Negotiation)...');
        console.log(`🔌 Verifying Gmail SMTP Connection via Nodemailer...`);
        let verifyResult = false;
        let verifyError = null;
        
        try {
            verifyResult = await transporter.verify();
            console.log(`✅ SMTP Verification SUCCESS: ${verifyResult}`);
        } catch (verifyErr) {
            console.error(`❌ SMTP Verification FAILED: ${verifyErr.code || verifyErr.name} - ${verifyErr.message}`);
            verifyError = `${verifyErr.code || verifyErr.name}: ${verifyErr.message}`;
        }
        
        // LAYER 6: Summary and root cause analysis
        console.log('\n📊 LAYER 6: Connectivity Summary & Root Cause Analysis:');
        console.log(`  • DNS Resolution: ${dnsResult ? '✅ PASS' : '❌ FAIL'}`);
        console.log(`  • TCP Connection: ${tcpConnected ? '✅ PASS' : '❌ FAIL'} ${tcpError ? `(${tcpError})` : ''}`);
        console.log(`  • TLS Handshake: ${tlsConnected ? '✅ PASS' : '❌ FAIL'} ${tlsError ? `(${tlsError})` : ''}`);
        console.log(`  • SMTP Verification: ${verifyResult ? '✅ PASS' : '❌ FAIL'} ${verifyError ? `(${verifyError})` : ''}`);
        
        // Determine root cause
        if (!dnsResult) {
            console.error('\n🎯 ROOT CAUSE: DNS Resolution Failure');
            console.error('   → Cannot resolve smtp.gmail.com hostname');
            console.error('   → Check network DNS configuration or firewall DNS blocking');
        } else if (!tcpConnected) {
            console.error('\n🎯 ROOT CAUSE: TCP Connection Failure');
            console.error('   → DNS works, but cannot establish TCP connection to smtp.gmail.com:465');
            console.error('   → Likely: Render firewall blocks outbound port 465');
            console.error('   → Or: Gmail server rejected connection from Render IP');
        } else if (!tlsConnected) {
            console.error('\n🎯 ROOT CAUSE: TLS Handshake Failure');
            console.error('   → TCP connection works, but TLS negotiation fails');
            console.error('   → Likely: Certificate validation or TLS protocol mismatch');
            console.error('   → Or: Render firewall intercepts TLS negotiation');
        } else if (!verifyResult && verifyError) {
            console.error('\n🎯 ROOT CAUSE: SMTP Protocol Failure');
            console.error(`   → TLS works, but SMTP negotiation fails: ${verifyError}`);
            console.error('   → Likely: Authentication failure or SMTP capability mismatch');
            console.error('   → Check: GMAIL_USER and GMAIL_PASS environment variables');
        } else if (verifyResult) {
            console.log('\n🎯 ✅ ALL LAYERS PASS: Connection is healthy!');
            console.log('   → Ready to send email');
        }
        
        if (!verifyResult) {
            throw new Error(`❌ Transporter verification failed - Cannot establish Gmail SMTP connection. Root cause: ${verifyError || 'Unknown'}`);
        }
