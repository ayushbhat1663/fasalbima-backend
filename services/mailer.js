...
// 1. Transporter for Insurance Emails (Gmail)
...
async function sendInsuranceRequestEmail(data) {
    ...
    try {
        console.log('📤 Sending insurance request email via Gmail to:', adminEmail);
        const info = await transporter.sendMail(mailOptions);
        console.log('✅ Insurance request email sent:', info.messageId);
        return info;
    } catch (error) {
        console.error('❌ Gmail Error in sendInsuranceRequestEmail:', error.stack || error.message);
        throw error;
    }
}
