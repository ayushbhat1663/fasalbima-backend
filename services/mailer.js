...
const transporter = nodemailer.createTransport({
  ...
  connectionUrl: `smtp://${process.env.GMAIL_USER}:${process.env.GMAIL_PASS}@smtp.gmail.com:587/?pool=true`,
  pool: {
    maxConnections: 1,
    maxMessages: 5,
    rateDelta: 20000,
...
async function sendInsuranceRequestEmail(data, retryCount = 0) {
    ...
    try {
        ...
        const isTimeoutError = errorCode === 'ETIMEDOUT' || errorCode === 'ECONNREFUSED' || errorCode === 'EHOSTUNREACH' || error.message.includes('timeout');
        
        console.error(`❌ Gmail Error in sendInsuranceRequestEmail (${errorCode}):`, error.message);
        
        // Retry on transient network errors
        if (isTimeoutError && retryCount < maxRetries) {
            console.warn(`⚠️ Transient network error detected. Retrying in 2 seconds... (Attempt ${retryCount + 2}/${maxRetries + 1})`);
